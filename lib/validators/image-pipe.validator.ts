...
export const DEFAULT_MIMETYPES_SUPPORT = /^image\/(webp|avif|png|gif|jpeg|jpg)$/;
...
.addFileTypeValidator({ fileType: options?.mimeTypes ? new RegExp(`^image\\/(${options.mimeTypes.join("|")})$`) : DEFAULT_MIMETYPES_SUPPORT })
